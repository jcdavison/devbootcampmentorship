class Authorization < ActiveRecord::Base
  ...
  def update_info(auth_hash, role)
    self.update_attributes(provider: auth_hash[:provider], email: auth_hash[:info][:email], uid: auth_hash[:uid], token: auth_hash[:credentials][:token], secret: auth_hash[:credentials][:secret]) 
